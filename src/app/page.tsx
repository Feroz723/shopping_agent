  ...
  return (
    <main>
      <nav className="nav shell">
        <a className="brand" href="#top" aria-label="Scout home"><span className="brand-mark">S</span>scout</a>
      ...
      <section className={`hero shell ${result ? "hero-compact" : ""}`} id="top">
        ...
        <p className="hero-copy">Tell Scout what you need. It weighs price, real shopper feedback, and retailer value to surface the right options.</p>
        <form className="search-box" onSubmit={submit}>
          <span className="search-icon" aria-hidden="true">⌕</span>
          <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Try “best workout shoes under $100”" aria-label="What are you shopping for?" />
          <button type="submit" disabled={loading || query.trim().length < 3}>{loading ? "Scouting..." : "Scout it"}<span aria-hidden="true">→</span></button>
        </form>
      ...
      {result && !loading && (
        <section className="results shell" aria-live="polite" id="results">
          <div className="results-heading">
            <div><p className="eyebrow"><span /> Your scouting report</p><h2>Top matches for <em>“{result.query}”</em></h2></div>
          ...
          <div className="insight-card"><div className="spark">✦</div><div><p className="insight-label">Scout’s take</p><p>{result.summary}</p></div></div>
