...
async function testFreeSearch() {
  ...
  if (budget1 !== 1000) throw new Error(`Budget extraction failed for 'shoes under 1000 rupees', got: ${budget1}`);
  const budget2 = extractBudgetFromQuery("headphones under $150");
  if (budget2 !== 150) throw new Error(`Budget extraction failed for 'headphones under $150', got: ${budget2}`);

  // Test 2: findProducts returns valid results for INR budget query
  ...
  // Test 3: braveFallbackProducts fallback function directly
